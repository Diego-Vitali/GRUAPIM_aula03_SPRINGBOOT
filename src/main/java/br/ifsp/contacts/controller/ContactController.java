...
public class ContactController {
    ...
    // EXCERCÍCIO 1
    // endpoint para buscar contato por nome

    @GetMapping("/search")
    public List<Contact> buscaContatoPeloNome (@RequestParam String nome) {
        return contactRepository.findByNomeContainingIgnoreCase(nome);
    }

    // EXERCERCÍCIO 2
    // endpoint patch para atualizar contatos
    ...
    public Contact atualizaContatoPorCampo (@PathVariable Long id, @RequestBody Contact contact){
        ...
        if (contact.getTelefone() != null) {
            contatoExistente.setTelefone(contact.getTelefone());
        }
        if (contact.getEmail() != null) {
            contatoExistente.setEmail(contact.getEmail());
        }
